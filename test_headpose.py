...
def get_cl_args():
    ...
    parser.add_argument("--n_points", default=8, type=int, help = "How many points?")
...
marks_3d_to_2d_map = np.array((0, 3, 4, 5, 6, 7), dtype=np.uint32)
...
def draw_headpose(img, marks):
    ...
    marks = marks[marks_3d_to_2d_map, :].astype(np.float64)
    camera_mat = getCameraMatrix(img.shape)
    dist_coeffs = np.zeros((4, 1))
    success, rotationVec, translationVec = cv2.solvePnP(marks_3d, marks, camera_mat,
